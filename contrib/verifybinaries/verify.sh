...
###   CleanCoincore.org and CleanCoin.org and compares them.
...
WORKINGDIR="/tmp/CleanCoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://CleanCoincore.org"
HOST2="https://CleanCoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="CleanCoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'CleanCoin-',
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "CleanCoin.org failed to provide signature file, but CleanCoincore.org did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "CleanCoin.org and CleanCoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the CleanCoin Core binary release signing key installed?"
   fi
