...
The CleanCoin Core daemon can be configured to act as a trusted "border
router", implementing the CleanCoin wire protocol and relay, making
...
The ZeroMQ feature in CleanCoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building CleanCoind:
...
For instance:
    ...
    $ CleanCoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/CleanCoind.tx.raw \
...
These options can also be provided in CleanCoin.conf.
...
From the perspective of CleanCoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into CleanCoind directly. Furthermore, no information is
...
using. CleanCoind appends an up-counting sequence number to each
