...
CleanCoin Core version 0.20.0 is now available from:

  <https://CleanCoincore.org/bin/CleanCoin-core-0.20.0/>
...
using the Linux kernel, macOS 10.12+, and Windows 7 and newer.  CleanCoin
Core should also work on most other Unix-like systems but is not as
frequently tested on them.  It is not recommended to use CleanCoin Core on
unsupported systems.

From CleanCoin Core 0.20.0 onwards, macOS versions earlier than 10.12 are no
longer supported. Additionally, CleanCoin Core does not yet change appearance
...
- Instead of running `make` simply, you should instead run
  `CleanCoin_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from CleanCoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since CleanCoin Core version 0.18.0.
...
- Testing or debugging of implementations of the CleanCoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of CleanCoin Core. CleanCoin Core logs debug messages
...
- OpenSSL is no longer used by CleanCoin Core.  (#17265)

- BIP70 support has been fully removed from CleanCoin Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/CleanCoin/CleanCoin/issues/16599),
  [PR #16702](https://github.com/CleanCoin/CleanCoin/pull/16702), and the
  `CleanCoind help` for more information.  This option is experimental and
...
- All custom settings configured when CleanCoin Core starts are now
...
- The "Start CleanCoin Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://CleanCoincore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple CleanCoin Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed CleanCoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/CleanCoin-core/HWI) for
  signing.  Future versions of CleanCoin Core should support a GUI option
...
- CleanCoin Core's automatically-generated source code documentation is
  now available at https://doxygen.CleanCoincore.org.  (#17596)
...
- The `CleanCoin-cli` utility used with the `-getinfo` parameter now
...
- The `CleanCoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `CleanCoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_CleanCoin-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBCleanCoin_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_CleanCoin` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBCleanCoin_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for CleanCoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate CleanCoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest CleanCoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_CleanCoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (CleanCoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone CleanCoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about CleanCoin Core files (hebasto)
...
- #17119 Fix broken CleanCoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from CleanCoin-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.CleanCoincore.org (laanwj)
...
- #17648 Rename wallet-tool references to CleanCoin-wallet (hel-o)
...
- #18341 Replace remaining literal CLN with `CURRENCY_UNIT` (domob1812)
...
- #16115 On CleanCoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from CleanCoin-cli -getinfo (malevolent)
...
[Transifex](https://www.transifex.com/CleanCoin/CleanCoin/).
