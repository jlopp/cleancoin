CleanCoin Core version 0.19.0.1 is now available from:

  <https://CleanCoincore.org/bin/CleanCoin-core-0.19.0.1/>
...
installer (on Windows) or just copy over `/Applications/CleanCoin-Qt` (on Mac)
or `CleanCoind`/`CleanCoin-qt` (on Linux).

Upgrading directly from a version of CleanCoin Core that has reached its EOL is
possible, but might take some time if the datadir needs to be migrated.  Old
wallet versions of CleanCoin Core are generally supported.

Compatibility
==============

CleanCoin Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use CleanCoin Core on unsupported systems.
...
to open the CleanCoin Core .dmg. This is due to new signing requirements
imposed by Apple, which the CleanCoin Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/CleanCoin/CleanCoin/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running CleanCoin Core on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of CleanCoin
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `CleanCoind -help` output for these two options for
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile CleanCoin Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in CleanCoin Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of CleanCoin Core. (#14054)

- To eliminate well-known denial-of-service vectors in CleanCoin Core,
  ...
  This prevents CleanCoin Core from sending the BIP111 NODE_BLOOM service
  ...
  of CleanCoin Core and nodes that have manually enabled BIP37 support,
...
- By default, CleanCoin Core will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)

Miscellaneous CLI Changes
-------------------------

- The `testnet` field in `CleanCoin-cli -getinfo` has been renamed to
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how CleanCoin Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  CleanCoin Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, CleanCoin Core will be bound to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 CleanCoin address without
...
- When creating a transaction with a fee above `-maxtxfee` (default 0.1
  CLN), the RPC commands `walletcreatefundedpsbt` and
  `fundrawtransaction` will now fail instead of rounding down the fee.
  Be aware that the `feeRate` argument is specified in CLN per 1,000
  vbytes, not greta per vbyte. (#16257)
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in CleanCoin Core v0.17.0), this can eliminate a
...
- #15649 Add ChaCha20Poly1305@CleanCoin AEAD (jonasschnelli)
...
- #15639 CleanCoin-wallet tool: Drop libCleanCoin_server.a dependency (ryanofsky)
...
- #16521 Use the default maxfeerate value as CLN/kB (Remagpie)
...
- #16366 init: Use InitError for all errors in CleanCoind/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "CleanCoin-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of CleanCoind build (NicolasDorier)
...
- #16441 remove qt libjpeg check from CleanCoin_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_CleanCoin to auto generated project files (sipsorcery)
...
- #15773 Add CleanCoinTestFramework::sync_* methods (MarcoFalke)
...
- #15352 Reduce noise level in test_CleanCoin output (practicalswift)
...
- #15896 feature_filelock, interface_CleanCoin_cli: Use PACKAGE_NAME in messages rather than hardcoding CleanCoin Core (luke-jr)
...
- #16277 Suppress output in test_CleanCoin for expected errors (gertjaap)
...
- #16929 follow-up to rpc: default maxfeerate value as CLN/kB (jonatack)
...
- #16489 log: harmonize CleanCoind logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (CleanCoinVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example CleanCoin.conf (fanquake)
...
- #15683 Comment for seemingly duplicate LIBCleanCoin_SERVER (Bushstar)
...
- #16080 build/doc: update CleanCoin_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in CleanCoin.conf (fanquake)
...
- #16621 add default CleanCoin.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the CleanCoin-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, CleanCoin.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/CleanCoind.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/CleanCoin/CleanCoin/).
