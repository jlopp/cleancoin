...
CleanCoin Core version 0.19.0 is now available from:

  <https://CleanCoincore.org/bin/CleanCoin-core-0.19.0/>
...
- The Tor onion service that is automatically created by setting the
  ...
  `-datadir`) and can be removed if not needed. CleanCoin Core will no longer
...
RPC commands have been updated to include two new fee estimation methods "CLN/kB" and "sat/B".
...
- The `connections` field of `CleanCoin-cli -getinfo` is expanded to return a JSON
  object with `in`, `out` and `total` numbers of peer connections. It previously
  returned a single integer value for the total number of peer connections. (#19405)

- A new `CleanCoin-cli -generate` command, equivalent to RPC `generatenewaddress`
  followed by `generatetoaddress`, can generate blocks for command line testing
  purposes. This is a client-side version of the
  former `generate` RPC. See the help for details. (#19133)

- The `CleanCoin-cli -getinfo` command now displays the wallet name and balance for
...
- The `startupnotify` option is used to specify a command to
  execute when CleanCoin Core has finished with its startup
  sequence. (#15367)
...
- The `-salvagewallet` startup option has been removed. A new `salvage` command
  has been added to the `CleanCoin-wallet` tool which performs the salvage
...
CleanCoin Core will no longer automatically create new wallets on startup. It will
load existing wallets specified by `-wallet` options on the command line or in
`CleanCoin.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
CleanCoin Core will now just log warnings instead of creating new wallets with
new keys and addresses like previous releases did.

New wallets can be created through the GUI (which has a more prominent create
wallet option), through the `CleanCoin-cli createwallet` or `CleanCoin-wallet
...
Bugs and missing functionality can be reported to the [issue tracker](https://github.com/CleanCoin/CleanCoin/issues).
...
Users who already have a CleanCoin Core wallet can continue to use it as they did before without
any change in behavior. Newly created Legacy Wallets (which is the default type of wallet) will
behave as they did in previous versions of CleanCoin Core.
...
Default Wallet created upon first startup of CleanCoin Core will be a Legacy Wallet.
...
The change to using descriptors changes the default derivation paths used by CleanCoin Core
...
- Wallets created or loaded in the GUI will now be automatically loaded on
  startup, so they don't need to be manually reloaded next time CleanCoin Core is
  started. The list of wallets to load on startup is stored in
  `\<datadir\>/settings.json` and augments any command line or `CleanCoin.conf`
