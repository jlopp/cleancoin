...
void ReceiveCoinsDialog::setModel(WalletModel *_model)
{
    ...
    if(_model && _model->getOptionsModel())
    {
        ...
        ui->addressTypeSelector->setModel(new CleanCoinAddressTypes(this));
        
        // user explicitly set the type, use it
        if (model->wallet().getDefaultAddressType() == OutputType::BECH32) {
            ui->addressTypeSelector->setCurrentIndex(CleanCoinAddressType::NATIVE_SEGWIT);
        } else if (model->wallet().getDefaultAddressType() == OutputType::P2SH_SEGWIT){
            ui->addressTypeSelector->setCurrentIndex(CleanCoinAddressType::NESTED_SEGWIT);
        } else {
            ui->addressTypeSelector->setCurrentIndex(CleanCoinAddressType::LEGACY);
        }
...
void ReceiveCoinsDialog::on_receiveButton_clicked()
{
    ...
    CleanCoinAddressType address_type_selected = (CleanCoinAddressType)ui->addressTypeSelector->currentIndex();
    if (CleanCoinAddressType::NATIVE_SEGWIT == address_type_selected) {
        address_type = OutputType::BECH32;
    } else if (CleanCoinAddressType::NESTED_SEGWIT == address_type_selected) {
...
void ReceiveCoinsDialog::copyURI()
{
    ...
    const QString uri = GUIUtil::formatCleanCoinURI(submodel->entry(sel.row()).recipient);
