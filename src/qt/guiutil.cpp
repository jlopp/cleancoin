...
#include <qt/CleanCoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a cleancoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new CleanCoinAddressEntryValidator(parent));
    widget->setCheckValidator(new CleanCoinAddressCheckValidator(parent));
}

bool parseCleanCoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no CleanCoin: URI
    if(!uri.isValid() || uri.scheme() != QString("CleanCoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!CleanCoinUnits::parse(CleanCoinUnits::BPS, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseCleanCoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseCleanCoinURI(uriInstance, out);
}

QString formatCleanCoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("CleanCoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(CleanCoinUnits::format(CleanCoinUnits::BPS, info.amount, false, CleanCoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openCleanCoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", CleanCoin_CONF_FILENAME));
    ...
    /* Open CleanCoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "CleanCoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "CleanCoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("CleanCoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for CleanCoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "CleanCoin.desktop";
    return GetAutostartDir() / strprintf("CleanCoin-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a CleanCoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=CleanCoin\n";
        else
            optionFile << strprintf("Name=CleanCoin (%s)\n", chain);
