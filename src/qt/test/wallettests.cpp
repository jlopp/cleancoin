...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<CleanCoinAmountField*>("payAmount")->setValue(amount);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_CleanCoin-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_CleanCoin-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_CleanCoin-qt  # macOS
void TestGUI(interfaces::Node& node)
{
...

    {
        ...
        QString balanceComparison = CleanCoinUnits::formatWithUnit(unit, balance, false, CleanCoinUnits::SeparatorStyle::ALWAYS);
    ...
    // Amount input
    CleanCoinAmountField* amountInput = receiveCoinsDialog.findChild<CleanCoinAmountField*>("reqAmount");
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QCOMPARE(uri.count("CleanCoin:"), 2);
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_CleanCoin-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
