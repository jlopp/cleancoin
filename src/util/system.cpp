...
const char * const CleanCoin_CONF_FILENAME = "CleanCoin-pos.conf";
const char * const CleanCoin_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/CleanCoin/CleanCoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //CleanCoin-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", CleanCoin_SETTINGS_FILENAME);
...
#else
    const char* pszModule = "CleanCoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows: C:\Users\Username\AppData\Roaming\CleanCoin-pos
    // macOS: ~/Library/Application Support/CleanCoin-pos
    // Unix-like: ~/.CleanCoin-pos
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CleanCoin-pos";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/CleanCoin-pos";
#else
    // Unix-like
    return pathRet / ".CleanCoin-pos";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", CleanCoin_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure CleanCoin Core copyright is not removed by accident
    if (copyright_devs.find("CleanCoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The CleanCoin Core developers";
    }
