...
class FullBlockTest(CleanCoinTestFramework):
    ...
    def run_test(self):
        ...
        #    Low-S and High-S values (whereas the CleanCoin code has custom code which does so);
    ...
    def next_block(self, number, spend=None, additional_coinbase_value=0, script=CScript([OP_TRUE]), *, version=4):
        ...
        if spend is None:
            ...
            coinbase.vout[0].nValue += spend.vout[0].nValue - 1  # all but one greta to fees
            coinbase.rehash()
            block = create_block(base_block_hash, coinbase, block_time, version=version)
            tx = self.create_tx(spend, 0, 1, script)  # spend 1 greta
            self.sign_tx(tx, spend)
