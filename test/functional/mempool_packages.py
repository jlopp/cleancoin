...
class MempoolPackagesTest(CleanCoinTestFramework):
    ...
    def chain_transaction(self, node, parent_txid, vout, value, fee, num_outputs):
        send_value = greta_round((value - fee)/num_outputs)
    ...
    def run_test(self):
        ...
        for x in reversed(chain):
            descendant_fees += mempool[x]['fee']
            if (x == chain[-1]):
                assert_equal(mempool[x]['modifiedfee'], mempool[x]['fee']+greta_round(0.00002))
                assert_equal(mempool[x]['fees']['modified'], mempool[x]['fee']+greta_round(0.00002))
            assert_equal(mempool[x]['descendantfees'], descendant_fees * COIN + 2000)
            assert_equal(mempool[x]['fees']['descendant'], descendant_fees+greta_round(0.00002))
        ...
        send_value = greta_round((value - fee)/2)
