...
class WalletTest(CleanCoinTestFramework):
    ...
    def run_test(self):
        ...
        fee_rate_sat_vb = 2
        fee_rate_CLN_kvb = fee_rate_sat_vb * 1e3 / 1e8
        explicit_fee_rate_CLN_kvb = Decimal(fee_rate_CLN_kvb) / 1000
        ...
        node_2_bal = self.check_fee_amount(balance, node_2_bal - Decimal('10'), explicit_fee_rate_CLN_kvb, self.get_vsize(self.nodes[2].gettransaction(txid)['hex']))
        ...
        for target, mode in product([-1, 0], ["CLN/kb", "sat/b"]):
        ...
        if not self.options.descriptors:
            ...
            # This will raise an exception for attempting to get the private key of an invalid CleanCoin address
            assert_raises_rpc_error(-5, "Invalid CleanCoin address", self.nodes[0].dumpprivkey, "invalid")

            # This will raise an exception for attempting to set a label for an invalid CleanCoin address
            assert_raises_rpc_error(-5, "Invalid CleanCoin address", self.nodes[0].setlabel, "invalid address", "label")

            # This will raise an exception for importing an invalid address
            assert_raises_rpc_error(-5, "Invalid CleanCoin address or script", self.nodes[0].importaddress, "invalid")
            ...
            assert_fee_amount(fee, tx_size, Decimal(fee_rate_CLN_kvb))
